...
var framerate = 300; // how many msec to wait between frame streams
...
module.exports = {
    ...
    init: function(expapp, config) {
        ...
        video_stream.on('data', function(matrix){
			...
			if (curtime > (last_frame + framerate)) {
				matrix.convertGrayscale();
				matrix.pyrDown();
	            video_buffer = matrix.toBuffer();
				last_frame = curtime;
        		video_stream.emit("writeframe");
			}
        });
        ...
        expapp.get('/video', function(request, response) {
            ...
            // wait until we have a frame we want and then send it.
            video_stream.on('writeframe', function(){
